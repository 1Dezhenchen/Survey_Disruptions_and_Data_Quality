...
- **rpart** [@rpart]: A comprehensive package for building recursive partitioning models, commonly used for creating decision trees. Particularly valuable for classification and regression tasks, it supports model visualization and interpretation, offering control over tree complexity and pruning for enhanced model performance.
...
Although the difference in accuracy is modest, the Bayesian GLM aligns more closely with the overarching objective of optimizing predictive performance across all classes. Furthermore, the Bayesian GLM's probabilistic framework provides a nuanced interpretation of predictions, capturing uncertainty and enabling more robust decision-making. Therefore, despite the decision tree's relative strength in minority class detection, the Bayesian GLM was selected as the final model due to its superior overall accuracy and its ability to provide a more balanced representation of both majority and minority classes. 
```{r}
#| echo: false
#| eval: true
#| message: false
